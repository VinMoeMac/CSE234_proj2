...
We wrote 105 hand-authored synthetic NL question / schema-link pairs targeting the most under-represented databases:
\begin{itemize}[noitemsep,topsep=2pt]
...
We ran 30+ distinct configurations over three phases: (1) RapidFire AI multi-config grid for initial exploration, (2) \texttt{train\_simple.py} single-config runs for fine-grained control, and (3) inference-time ablations on the best adapter.
...
+ camelCase split & 0.499 & 0.419 \\
+ two-stage inference & 0.499 & \textbf{0.440} \\
+ $r=32$, lr=$8\times10^{-5}$ & 0.517 & 0.454 \\
+ synthetic NTSB (46 ex) & 0.499 & \textbf{0.454} \\
\bottomrule
\end{tabular}
\end{center}
...
Claude Code (\texttt{claude-sonnet-4-6} 1M context) was used extensively throughout this project via the VS Code extension: for writing and debugging training/inference scripts, generating synthetic training data, diagnosing failure modes, and drafting this report. All experimental design decisions, hyperparameter choices, and interpretations of results were made by the author. The AI assistant was used as a coding and writing accelerator, not as a substitute for judgment.
